...
|--------|-----|-------------|
| **Anders** | Executor Agent | Recibe solicitudes de acciones operativas (como la generación de reportes o renderizado de facturas) y las ejecuta interactuando con servicios externos como la Azure Function `FxContosoRetail`. |
| **Julie** | Planner Agent | Toma las señales analíticas y elabora planes de acción concretos. Coordina qué pasos deben ejecutarse para cumplir los objetivos del negocio. |
...
├─────────────────────┤
│  Microsoft Fabric   │  ← Capa de datos (Mark)
└─────────────────────┘
```
...
| Lab 4 | [Julie — Planner Agent](lab04-julie-planner-agent.md) | Crear el agente planificador que genera planes de acción. |
...
Si no estás siguiendo toda la secuencia de laboratorios, aquí solo se usa una base SQL para consultas del Lab 4. Puedes apuntar directamente a una base SQL standalone (por ejemplo Azure SQL Database) usando:
...
└── code/
   ...
   │       ├── FxContosoRetail.cs          ← Endpoints: HolaMundo, OrdersReporter, SqlExecutor
    ...
    │       └── ...
    ├── agents/
    │   └── AndersAgent/                   ← Console App: Agente Anders (Azure AI Agents Persistent)
    │       ├── Program.cs
    │       ├── appsettings.json
    │       └── ...
    └── tests/
