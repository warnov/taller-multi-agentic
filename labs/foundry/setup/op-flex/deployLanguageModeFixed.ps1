...
# ============================================================================
# Uso:
#   .\deployLanguageModeFixed.ps1 -TenantName "mi-tenant-temporal"
#   .\deployLanguageModeFixed.ps1 -TenantName "mi-tenant-temporal" -FabricWarehouseSqlEndpoint "<endpoint-sql-fabric>" -FabricWarehouseDatabase "<database>"
#   .\deployLanguageModeFixed.ps1 -TenantName "mi-tenant-temporal" -Location "eastus" -FabricWarehouseSqlEndpoint "<endpoint-sql-fabric>" -FabricWarehouseDatabase "<database>"
# ============================================================================
...
# FIX (Constrained Language Mode): [Console]::OutputEncoding y [System.Text.Encoding]::UTF8
# son tipos .NET no permitidos en CLM. Se envuelven en try/catch para que no detengan la ejecución.
...
'@
    ...
    # FIX (Constrained Language Mode): [System.IO.File]::WriteAllText con [System.Text.UTF8Encoding]::new()
    # no está permitido en CLM. Se reemplaza por Set-Content con -Encoding utf8NoBOM (PS 6+).
    Set-Content -Path $suffixTempFileForPreserve -Value $suffixTemplateForPreserve -Encoding utf8NoBOM
...
# FIX (Constrained Language Mode): igual que arriba, se usa Set-Content en lugar de [System.IO.File]::WriteAllText.
Set-Content -Path $suffixTempFile -Value $suffixTemplate -Encoding utf8NoBOM
...
for ($i = 0; $i -lt 30; $i++) {
    try {
        # FIX (Constrained Language Mode): [System.Net.Dns]::GetHostAddresses() no está permitido en CLM.
        # Se reemplaza por Resolve-DnsName, cmdlet nativo de PowerShell disponible en Windows.
        Resolve-DnsName -Name $scmHost -ErrorAction Stop | Out-Null
        $dnsReady = $true
        break
    } catch {
